...
function animate()
{
 yoel_engine_p.update();
	 setInterval(yoel_engine_p.update, 1000 / 18);
...
y_entity_p.move_by=function(x,y,z,solid)
{
	...
	if(!this.div){return;}//if no div escape
      ...
      // this.div.style.transform  = "translate("+this.x2+"px,"+this.y2+"px)";
...
y_entity_p.colide=function(type)
{
	...
	for(var i = 0; entity_list_len>=i ; i++)
	{
		...
		if(type == entity_type && entity_list[i] !=this )
		{ 
...
y_entity_p.camera_move =function(dir,type)
{
	...
	for(var i = 0; entity_list_len>=i ; i++)
	{
		e= entity_list[i];
		do_stuff=true;
		//only move if it dosnt eaquel to type or its no cam
		if(e.type !=type && !e.no_cam)
		{
			
			if(dir == "up"){e.y += this.speed; }
			if(dir == "down"){e.y -= this.speed; }
			if(dir == "left"){e.x += this.speed; }
			if(dir == "right"){e.x -= this.speed; }
		}
		y_entity_p.move_by.call(e,0,0);//updatexy
	}//end loop
	if(dir == "up"){ this.world.cam.y +=this.speed;}
	if(dir == "down"){ this.world.cam.y -=this.speed;}
	if(dir == "left"){ this.world.cam.x -=this.speed;}
	if(dir == "right"){ this.world.cam.x +=this.speed;}
}//end camera_move
...
{
	this.finished = false;//auto rewind
...
y_animation_p.play = function(animation)
{
	...
	if(this.play_current_frame >= this.animations[animation].length){this.play_current_frame = this.animations[animation].length-1;}
	//get current farme from current animation
	current_frame = this.animations[animation][this.play_current_frame];

	this.go_to_frame(current_frame,true)
	this.play_current_frame++;
	
	//finished
	if(this.play_current_frame >= this.animations[animation].length)
	{
		this.finished = true;
		this.play_current_frame =0;
	}
	//rewind if needed
	if(this.rewind && this.play_current_frame >= this.animations[animation].length){this.play_current_frame =0; this.finished = false;}
}//end animate
//////////////////end dom_animation/////////////
